...
(defun psql-recreate-tables (&optional db)
  (let ((database (or db "metis"))
    ...
    (psql-do-query "create table if not exists flows (id serial, version integer, account_id integer, interface_id integer, srcaddr integer, dstaddr integer, srcport integer, dstport integer, protocol integer, packets integer, bytez integer, start integer, endt integer, action integer, status integer)" database)
