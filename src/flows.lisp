...
(manardb:defmmclass host ()
  ((ip :initarg :ip :reader ip :accessor host-ip)
   (name :initarg :name :reader name :accessor host-name)
   (internal :initarg :internal :reader internal :accessor host-internal)
   (dates :initarg :dates :reader dates :accessor host-dates) ;;sxhash
   (partners :initarg :partners :reader partners :accessor host-partners) ;;sxhash
   (srcports :initarg :srcports :reader srcports :accessor host-srcports) ;;sxhash
   (dstports :initarg :dstports :reader dstports :accessor host-dstports) ;;sxhash
   ))

(manardb:defmmclass port ()
  (
   (number :initarg :number :reader number :accessor port-number)
   (service :initarg :service :reader service :accessor port-service)
   ))
...
(defun add-conversation (entry)

  )

(defun make-srcaddr (&rest rest) ;;(srcaddr-i dstaddr srcport dstport protocol packets bytez start endf action status)
  (let ((entry nil)
	(srcaddr-i (car rest)))
    (manardb:doclass (x 'metis::srcaddr :fresh-instances nil)
		     (with-slots (srcaddr hostname dstaddrs) x
			   ...
			   (setf entry x))))
    (if (null entry)
	(setf entry (make-instance 'metis::srcaddr :srcaddr srcaddr-i :hostname (get-hostname-by-ifp srcaddr-i))))
    (format t "~% srcaddr:~A dstip:~A srcport:~A dstport:~A" srcaddr-i dstaddr srcport dstport)
    (make-dstaddr (cdr rest))))

(defun make-dstaddr (&rest rest)
  (let ((entry nil)
	(dstaddr-i (car rest)))
    (manardb:doclass (x 'metis::dstaddr :fresh-instances nil)
		     (with-slots (dstaddr hostname srcport) x
		       (if (usocket:ip= dstaddr dstaddr-i)
			   (setf entry x))))
    (if (null entry)
	(setf entry (make-instance 'metis::dstaddr :dstaddr dstaddr-i :hostname (get-hostname-by-ifp srcaddr-i))))
    (format t "~% srcaddr:~A dstip:~A srcport:~A dstport:~A" srcaddr-i dstaddr srcport dstport)
    (make-srcport (cdr rest))))

(defun make-srcport (&rest rest)
  (let ((entry nil)
	(value (car rest)))
    (manardb:doclass (x 'metis::srcport :fresh-instances nil)
		     (with-slots ( hostname dstaddrs) x
		       (if (usocket:ip= srcaddr srcaddr-i)
			   (setf entry x))))
    (if (null entry)
	(setf entry (make-instance 'metis::srcaddr :srcaddr srcaddr-i :hostname (get-hostname-by-ifp srcaddr-i))))
    (format t "~% srcaddr:~A dstip:~A srcport:~A dstport:~A" srcaddr-i dstaddr srcport dstport)
    (make-dstport (cdr rest))))

(defun make-dstport (&rest rest)
  (let ((entry nil)
	(new-value (car rest)))
    (manardb:doclass (x 'metis::dstaddr :fresh-instances nil)
		     (with-slots (dstport name protocol) x
		       (if (eq new-value dstport)
			   (setf entry x))))
    (if (null entry)
	(setf entry (make-instance 'metis::dstport :dstport new-value :protocol protocol)))
    (format t "~% srcaddr:~A dstip:~A srcport:~A dstport:~A" srcaddr-i dstaddr srcport dstport)
    (make-protocol (cdr rest))))

(defun make-protocol (&rest rest)
  (let ((entry nil)
	(new-value (car rest)))
    (manardb:doclass (x 'metis::protocol :fresh-instances nil)
		     (with-slots (protocol name conversation) x
		       (if (eq new-value protocol)
			   (setf entry x))))
    (if (null entry)
	(setf entry (make-instance 'metis::protocol :protocol new-value :name "foo")))
    (format t "~% srcaddr:~A dstip:~A srcport:~A dstport:~A" srcaddr-i dstaddr srcport dstport)
    (make-conversation (cdr rest))))
