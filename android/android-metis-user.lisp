...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; Examples of proxies that implement the MetisServer that is declared
;;; in ./MetisDemo/src/com/lispworks/example/metisdemo/Metis.java, and is
...
(defun call-metis-clicked-full(where)
...
(lw-ji:define-lisp-proxy lisp-metis-server-full
  ("com.lispworks.example.metisdemo.Metis.MetisServer"
   ("init" init-metis)
   ("playSquare" call-metis-clicked-full)
   ("undoMove" undo-move)
...
(lw-ji:define-lisp-proxy lisp-metis-server-lazy
  ("com.lispworks.example.metisdemo.Metis.MetisServer")
  (:options :default-function lisp-metis-server-lazy-default-function))

;;; The default function for lisp-metis-server-lazy. We can get away
;;; with just checking the first character of the method name
(defun lisp-metis-server-lazy-default-function (method-name &optional arg)
...
;;; Match Java definition in ./MetisDemo/src/com/lispworks/example/metisdemo/Metis.java
(defconstant METIS_SERVER_TYPE_JAVA 0)        ; We do not expect to see this one
(defconstant METIS_SERVER_TYPE_FULL_PROXY 1)
(defconstant METIS_SERVER_TYPE_LAZY_PROXY 2)

;;; This one so Java can call it directly. 
(defun create-lisp-metis-server (which) 
  (let ((name (if (eq which  METIS_SERVER_TYPE_FULL_PROXY) 
                  'lisp-metis-server-full
                'lisp-metis-server-lazy)))
    (lw-ji:make-lisp-proxy name)))
       

;;; Ensure the symbols that are going to be called directly from Java
;;; are kept after delivering.
;;; Strictly speaking does not need to keep the Metis symbols, because they are
;;; referenced by the proxy.  We keep them here as a matter of principle. 

(deliver-keep-symbols  
 'init-metis 'metis-clicked 'undo-move 'set-computer-plays  ; Metis symbols
 'create-lisp-metis-server  ; proxy creation
 )
